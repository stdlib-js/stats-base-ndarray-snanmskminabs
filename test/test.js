...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof snanmskminabs, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( snanmskminabs.length, 1, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the minimum absolute value of a one-dimensional single-precision floating-point ndarray according to a mask, ignoring NaN values', function test( t ) {
	var mask;
	var x;
	var v;
...
tape( 'if provided an empty ndarray, the function returns `NaN`', function test( t ) {
	...
	v = snanmskminabs( [ vector( 'float32', x, 0, 1, 0 ), vector( 'uint8', mask, 0, 1, 0 ) ] );
	t.strictEqual( isnanf( v ), true, 'returns expected value' );
...
tape( 'if provided an ndarray containing a single element, the function returns the absolute value of the first element', function test( t ) {
	...
	v = snanmskminabs( [ vector( 'float32', x, 1, 1, 0 ), vector( 'uint8', mask, 1, 1, 0 ) ] );
...
tape( 'the function supports one-dimensional ndarrays having non-unit strides', function test( t ) {
	...
	v = snanmskminabs( [ vector( 'float32', x, 5, 2, 0 ), vector( 'uint8', mask, 5, 2, 0 ) ] );
	t.strictEqual( v, 1.0, 'returns expected value' );
	t.end();
});
...
tape( 'the function supports one-dimensional ndarrays having negative strides', function test( t ) {
	...
	v = snanmskminabs( [ vector( 'float32', x, 5, -2, 8 ), vector( 'uint8', mask, 5, -2, 8 ) ] );
	t.strictEqual( v, 1.0, 'returns expected value' );
	t.end();
});
...
tape( 'the function supports one-dimensional ndarrays having non-zero offsets', function test( t ) {
	...
	v = snanmskminabs( [ vector( 'float32', x, 5, 2, 1 ), vector( 'uint8', mask, 5, 2, 1 ) ] );
	t.strictEqual( v, 1.0, 'returns expected value' );
